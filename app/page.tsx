...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans uppercase overflow-x-hidden flex items-center justify-center p-4">
      {/* THE ORIGINAL SKELETON: Fixed max-width for that "App" feel */}
      <div className="w-full max-w-md border-x-4 border-white min-h-[95vh] flex flex-col p-6 bg-zinc-950 shadow-2xl relative">
        ...
        <div className="flex-grow">
          ...
          {/* STEP 3 & 4: THE TOOL HUB (RESTORED LIST STYLE) */}
          {(step === 3 || step === 4) && !showPay && (
            <div className="space-y-6 animate-in zoom-in">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-black italic uppercase tracking-tighter border-b-2 border-yellow-400">ANALYSIS: {niche || 'DOGS'}</h2>
                <button onClick={() => setStep(2)} className="text-yellow-400"><PencilLine className="w-5 h-5" /></button>
              </div>

              {/* LIST STYLE TOOLS FROM YOUR SCREENSHOT */}
              <div 
                onClick={() => { setSelectedTool("GAP FINDER"); setStep(4); }}
                className={`p-8 rounded-[40px] border-4 flex justify-between items-center transition-all ${selectedTool === "GAP FINDER" ? 'bg-white text-black border-yellow-400' : 'bg-black border-white'}`}
              >
                <div className="flex flex-col">
                  <Wrench className={`${selectedTool === "GAP FINDER" ? 'text-black' : 'text-yellow-400'} w-12 h-12 mb-2`} />
                  <p className="font-black text-2xl uppercase italic">GAP FINDER</p>
                </div>
                {selectedTool === "GAP FINDER" ? <Zap className="text-black" /> : <ChevronRight className="opacity-20" />}
              </div>

              <div className="p-8 bg-black rounded-[40px] border-4 border-white/20 flex justify-between items-center opacity-30 grayscale">
                <div className="flex flex-col">
                  <Lightbulb className="text-yellow-400 w-12 h-12 mb-2" />
                  <p className="font-black text-2xl uppercase italic text-white/50">PROFIT TOOL</p>
                </div>
              ...
              {step === 4 && (
                <button 
                  ...
                  className="w-full mt-6 bg-yellow-400 text-black py-8 rounded-[40px] font-black text-3xl border-4 border-white shadow-[0_10px_0_white] animate-in slide-in-from-bottom-5 uppercase italic"
                >
            ...
            <div className="py-6 flex flex-col items-center animate-in slide-in-from-bottom-10">
              <div className="bg-yellow-400 p-1 rounded-[50px] border-4 border-white shadow-2xl">
                <div className="bg-black rounded-[46px] p-10 text-center border-4 border-white">
                  <h3 className="text-5xl font-black mb-6 text-white italic leading-none uppercase tracking-tighter">UNLOCK<br/>RESULTS</h3>
                  <a href="https://buy.stripe.com/4gMbJ2cq85pl9oKfKobwk02" onClick={() => setTimeout(() => setStep(6), 2000)} className="block w-full bg-white text-black py-10 rounded-full font-black text-4xl text-center no-underline border-8 border-black shadow-[0_10px_0_#333] uppercase">PAY NOW</a>
              ...
              <button onClick={() => setShowPay(false)} className="mt-8 text-white/40 font-black text-xs italic underline">← BACK TO TOOLS</button>
            </div>
          )}
          ...
          {step >= 6 && (
            <div className="space-y-6 animate-in zoom-in">
              <div className="bg-white text-black p-4 rounded-2xl text-center font-black text-sm uppercase italic tracking-tighter">PAYMENT SUCCESSFUL</div>
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => {setStep(3); setShowPay(false);}} className="p-8 bg-black border-4 border-white rounded-[40px] flex flex-col items-center gap-4 active:bg-yellow-400 active:text-black">
                  <Wrench className="text-yellow-400 w-8 h-8" /><span className="text-[10px] font-black uppercase">NEW TOOL</span>
                </button>
                <button onClick={() => setStep(7)} className="p-8 bg-black border-4 border-white rounded-[40px] flex flex-col items-center gap-4 active:bg-yellow-400 active:text-black">
                  <BookOpen className="text-yellow-400 w-8 h-8" /><span className="text-[10px] font-black uppercase">HELP / FAQ</span>
                </button>
              </div>
            </div>
          )}
        ...
        <div className="mt-6 border-t-2 border-white/10 pt-6 text-center">
          <button onClick={() => {setStep(1); setNiche(""); setSelectedTool(""); setShowPay(false);}} className="text-[10px] font-black text-white/20 italic tracking-widest uppercase underline">RESET LAB TERMINAL</button>
        </div>
      </div>
    </div>
  );
