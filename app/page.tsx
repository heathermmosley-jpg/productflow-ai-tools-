...
export default function App() {
  ...
  // SECURE DESTINATIONS (NO BROKEN LINKS)
      ...
      <div className="w-full max-w-6xl border-x-8 border-white min-h-[90vh] flex flex-col p-6 md:p-12 bg-zinc-950 shadow-2xl relative">
        ...
        <div className="flex justify-between items-center py-6 border-b-4 border-white mb-8 sticky top-0 bg-zinc-950 z-50">
          ...
          <div className="flex items-center gap-4">
             <div className="hidden md:flex items-center gap-2 text-white/20 font-black text-[10px] tracking-widest">
               <ShieldCheck className="w-4 h-4" /> ENCRYPTION LEVEL: HIGH
             </div>
             <button onClick={() => handlePurchase(VAULT_URL)} className="bg-white text-black px-6 py-2 rounded-full font-black text-xs md:text-sm flex items-center gap-2 hover:bg-yellow-400 transition-all border-2 border-black">
        ...
        <div className="flex-grow">
          {step === 1 && (
            <div className="py-24 text-center animate-in zoom-in">
              <h2 className="text-7xl md:text-[12rem] font-black mb-12 italic text-yellow-400 leading-[0.8] tracking-tighter uppercase">ACCESS<br/>GRANTED.</h2>
              <button onClick={() => setStep(2)} className="bg-white text-black px-24 py-12 rounded-full font-black text-4xl border-4 border-black shadow-[0_12px_0_#facc15] active:translate-y-2 uppercase tracking-tighter">IDENTIFY NICHE</button>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-8 animate-in zoom-in">
              <div className="flex justify-between items-end border-b-4 border-yellow-400 pb-4">
                <h2 className="text-3xl md:text-6xl font-black italic uppercase tracking-tighter leading-none">TRANSFORMATION HUB</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 max-h-[50vh] overflow-y-auto pr-4 custom-scrollbar">
                {allTools.map((tool, i) => (
                  <div key={i} onClick={() => { setSelectedTool(tool); setStep(4); }} className="p-8 bg-zinc-900 border-4 border-white/10 rounded-[35px] hover:border-white hover:bg-black cursor-pointer transition-all group">
                    ...
                    <p className="text-[10px] font-bold opacity-30 mt-2 tracking-widest uppercase italic">READY FOR {niche || 'ACTION'}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {step === 5 && (
            <div className="py-20 text-center animate-in zoom-in space-y-12">
              <div className="inline-block bg-yellow-400 text-black px-12 py-4 rounded-full font-black text-4xl italic uppercase shadow-2xl">TUNNEL OPENED</div>
              ...
              <div className="flex flex-col md:flex-row gap-6 justify-center">
                <button onClick={() => setStep(3)} className="bg-white text-black px-16 py-10 rounded-full font-black text-3xl md:text-5xl border-4 border-black shadow-[0_12px_0_#facc15] uppercase italic flex items-center justify-center gap-4 hover:scale-105 active:translate-y-2">
                  <ArrowLeft className="w-10 h-10" /> KEEP SHOPPING
                </button>
                <button onClick={() => setStep(1)} className="bg-zinc-800 text-white px-12 py-10 rounded-full font-black text-2xl border-4 border-white/20 uppercase italic">NEW SCAN</button>
              </div>
            </div>
          )}
        </div>

        {/* FOOTER - CLEANER THAN EVER */}
        <div className="mt-8 pt-8 border-t-4 border-white/5 flex justify-between items-center opacity-20 font-black italic text-sm md:text-xl uppercase tracking-widest">
          <div className="flex items-center gap-2">
             <Lock className="w-5 h-5" /> END-TO-END SECURE
          </div>
